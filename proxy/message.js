...
exports.getMessagesCount = function (id, callback) {
    Message.count({master: id, has_read: false}, callback);
};
...
var getMessageRelations = exports.getMessageRelations = function (message, callback) {
    if (message.type === 'reply' || message.type === 'reply2' || message.type === 'at') {
        ...
        proxy.fail(callback);
        proxy.all('author', 'topic', 'reply', function (author, topic, reply) {
        ...
        User.getUserById(message.author, proxy.done('author'));
        Topic.getTopicById(message.topic, proxy.done('topic'));
        Reply.getReplyById(message.reply, proxy.done('reply'));
    } else {
 ...
 * @param {Function} callback 回调函数
 */
exports.getAllMessagesByUserId = function (userId, opt, callback) {
    Message.find({master: userId}, null, opt)
        .populate(['master', 'author', 'topic', 'reply'])
        .exec(callback);
};
/**
 * 获取关键词能搜索到的主题数量
...
exports.getReadMessagesByUserId = function (userId, callback) {
    Message.find({master: userId, has_read: true}, null,
...
exports.getUnreadMessageByUserId = function (userId, callback) {
    Message.find({master: userId, has_read: false}, null,
...
exports.updateMessagesToRead = function (userId, messages, callback) {
    ...
    var query = {master: userId, _id: {$in: ids}};
